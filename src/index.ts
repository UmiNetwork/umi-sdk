...
// /// List compiled module bundles as {package_name: [list of module names]}
// async function listCompiledContracts(
//   packagePath: string,
// ): Promise<Result<{ name: string; modules: string[] }[], ChainedError>> {
//   const path = Path.join(packagePath, 'build');

//   const readDirRes = await readDir(path);
//   if (readDirRes.isErr()) {
//     return err(new ChainedError(`Failed to list compiled bundle in ${path}`, readDirRes.error));
//   }
...
//       // Get all the compiled modules in the bundle
//       const readDirRes = await readDir(Path.join(entry.parentPath, entry.name, 'bytecode_modules'));
//       if (readDirRes.isErr()) {
//         return err(new ChainedError(`Failed to list compiled contracts in ${path}`, readDirRes.error));
//       }
//       const modules = [];
//       for (const module of readDirRes.value) {
//         if (module.isFile()) {
//           modules.push(module.name);
//         }
//       }
//       info.push({ name: parsed.name, modules });
//     }
//   }
//   return ok(info);
// }
...
//     const loadbytecodeRes = await loadBytecode(packagePath, packageName, module);
//     if (loadbytecodeRes.isErr()) {
//       return err(new ChainedError(`Failed to load bytecodes of ${module}`));
//     }

//     let bytecode = loadbytecodeRes.value;
//     if (!bytecode.startsWith('0x')) bytecode = '0x' + bytecode;
//     const sourcePath = Path.relative(hardhatRootPath, packagePath);
//     let artifact: Artifact = {
//       _format: 'hh-move-artifact-1',
//       contractName: module.substring(0, module.indexOf('.mv')),
